##########################################################################
#             TAREA 4 - ALGORITMOS DE BUSQUEDA INFORMADA
# LAB-BC1-10
...
def busquedas(problema, frontera, estrategia):
    ...
    visitados = Visitados()
    nID = 0
    h_inicio = heuristicaFuncion(problema.initState)
    valor_inicio = calcularValor(estrategia, id, 0, 0, h_inicio)
    nodo = Nodo(nID, 0, problema.initState, None, None, 0, h_inicio, valor_inicio)
            ...
            if not visitados.pertenece(auxNodo.estado) and auxNodo.profundidad < profundidadMax:
                ...
                if len(listSucesores) == 0 and frontera.nodosFrontera == []:
                    ...
                    for x in listSucesores:
                        ...
                        nID += 1
                        heuristica = heuristicaFuncion(x[1])
                        valor = calcularValor(estrategia, nID, auxNodo.profundidad+1, auxNodo.costo + x[2], heuristica)
                        #               ID      COSTO                   ESTADO  PADRE       ACCION  PROFUNDIDAD                 HEURISTICA   VALOR
                        nodo = Nodo (   nID,    auxNodo.costo + x[2],   x[1],   auxNodo,    x[0],   auxNodo.profundidad + 1,    heuristica,  valor)
                        if nodo.id == 68:
                            #nodo.padre.estado.listOfBottles
                            print(nodo.padre.id)
                            print(hashlib.md5(eliminarEspacios(str(nodo.padre.estado.listOfBottles)).encode()).hexdigest())
...
######################################################################################
# FUNCION HEURISTICA
# Contabilizar el número de líquidos no mezclados y las botellas vacías que 
# tenemos en un estado, y compararlo con el número de botellas totales del problema.
######################################################################################
...
def escribirFichero(nodo, nombre, estrategia,estado):
    cad = ""
...
def main():
    ...
    estrategias = {"anchura": "BREADTH", "profundidad acotada": "DEPTH", "coste uniforme": "UNIFORM", "voraz": "GREEDY", "a*": "A"}
    ...
    if estado0:
        ...
        if solucion:
            escribirFichero(solucion, nombreArch, estrategia, estado0)
